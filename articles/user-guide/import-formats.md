...
- A sample metadata file is required for both Annotation and
  [*Synthesis*](#synthesissyntheses) imports.
...
The sample metadata requirements for Annotations and
[*Syntheses*](#synthesissyntheses) are very similar. The primary
difference is that [*synthesis*](#synthesissyntheses) metadata must
include a [*campaignid*](#campaignid) column (Table 3). This is required
because a [*synthesis*](#synthesissyntheses) may combine data from
multiple [*Campaigns*](#campaign), and the [*campaignid*](#campaignid)
identifies the [*Campaign*](#campaign) associated with each
[*sample*](#sample).
...
[*Synthesis*](#synthesissyntheses) sample metadata file from a
stereo-BRUVs [*Campaign*](#campaign) that used additional backwards
facing cameras for habitat annotation. The opcode column is used to
define the [*sample*](#sample).

### Count

- A Count file is required for [*Synthesis*](#synthesissyntheses)
  imports.
...
**Table 4**. First four lines of an example Count file from two
stereo-BRUVs [*Campaigns*](#campaign) ready to be imported into a
[*Synthesis*](#synthesissyntheses). The opcode column is used to define
the [*sample*](#sample).
...
- A Length file is required for [*Synthesis*](#synthesissyntheses)
  imports.
...
- A Benthos Count file is required for
  [*Synthesis*](#synthesissyntheses) imports.
...
- A Benthos Length file is required for
  [*Synthesis*](#synthesissyntheses) imports.
...
- A Benthos Relief file is required for
  [*Synthesis*](#synthesissyntheses) imports.
